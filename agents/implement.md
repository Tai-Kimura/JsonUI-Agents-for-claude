...
- Edit the spec itself — route back to `define`. If a spec change is needed mid-implementation, stop and route.
- Edit `@generated` files — Protocol / Interface / ViewModelBase / VM method+var signatures / Repo+UseCase signatures / inheritance-list補完 / Kotlin `override` insertion. These are regenerated by `jui build` from the spec.
- Implement navigation (routing, `NavigationStack` / `NavHost` / Router) — delegate to `navigation-{ios,android,web}`.
...
You can't — add it to the spec via `define`, let `jui build` regenerate the Protocol. Then write the body here.
...
```
Please launch the `navigation-{ios|android|web}` agent (Phase 3 transitional: handle here in implement) with:
...
- Spec ↔ Impl drift on a method/var signature → spec wins; route to `define` to fix spec, then regenerate. Or add `@jui:protocol` marker if the signature should override.
...
- Spec is wrong → route back to `define` to fix the spec
...
### Ready for
- Test (route to `test` / `jsonui-test` agent for this screen)
```
...
- When you edit VM method bodies, don't introduce new public methods without going through `define` first.
...
6. **Touching spec mid-implementation without going through define** — routes around the `doc_validate_spec` gate. Always route to `define`.
...
```
Please launch the `test` agent (Phase 3 transitional: `jsonui-test`) for screen: {screen_name}
...
If multiple screens need implementing, return to conductor after each screen's test completes, so it can sequence the next.
