...
| 5 | Conditional / hand-written logic tested | `jsonui-test generate branch-tests --check` + `unit-stubs --check` exit 0 — **declared-vs-implemented agreement only**. Coverage of the declared API outcomes: reported by `jsonui-test validate` from 1.8.119, **a gate from 1.8.121** (nothing outside the app's baseline, nothing stale or vanished in it, nothing that cannot be baselined) | jsonui-test |
