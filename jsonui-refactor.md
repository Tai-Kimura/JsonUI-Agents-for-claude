...
> ```
>
> **KotlinJsonUI** - Call `DynamicComponentInitializer.initialize()` after `KotlinJsonUI.initialize()`:
> ```kotlin
> // In Application.onCreate() or MainActivity.onCreate()
> KotlinJsonUI.initialize(applicationContext)
> DynamicComponentInitializer.initialize()  // Auto-generated by kjui g adapter
> ```
> The generator automatically creates `DynamicComponentInitializer` in both debug and release source sets."
