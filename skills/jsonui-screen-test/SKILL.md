...
When layouts use `include` with `id`, child element IDs are prefixed: the include's `id` in camelCase, then the child's `id` in camelCase with its first letter capitalized.
...
// Actual ID becomes: "mainFormSubmitBtn"
```

**Test must use the full prefixed ID:**
```json
{ "action": "tap", "id": "mainFormSubmitBtn" }
```

Generated SwiftUI and Compose code spell it this way. Web does once its code is generated by `jui build` from jsonui-cli 1.8.121 with the project's rjui_tools synced (`jui sync_tool`); before that, or from a standalone `rjui build`, web keeps the child's own id (`submit_btn`). The dynamic mode spells it this way from SwiftJsonUI 10.28.0 and KotlinJsonUI 2.41.1; before them it keeps the include's id as written (`main_formSubmitBtn`; an include id without `_` is unaffected). UIKit and XML layouts are outside this and spell it their own way (UIKit: `mainForm_submit_btn`). While the platforms a test runs on still differ, limit the case with `platform`.
