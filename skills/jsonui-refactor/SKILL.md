...
**Note:** All layout edits MUST be made in the shared `layouts_directory`, NOT in platform-specific directories.
...
1. **Read all layout files** in the shared `layouts_directory` (NOT platform copies)
...
## Responsive Block Review

When reviewing layouts with `responsive` blocks:

### What to Check

1. **Valid size class keys** — Only: `compact`, `medium`, `regular`, `landscape`, `compact-landscape`, `medium-landscape`, `regular-landscape`
2. **No forbidden keys in overrides** — `type`, `child`, `data`, `responsive` must NOT appear inside responsive overrides
3. **Redundant overrides** — Remove overrides that match the default value
4. **Consistent patterns** — If multiple components have the same responsive changes, consider extracting a style with responsive

### Responsive Style Extraction

If the same responsive pattern appears in 3+ components, extract it:

**Before:**
```json
{ "fontSize": 14, "responsive": { "regular": { "fontSize": 20 } } }
{ "fontSize": 14, "responsive": { "regular": { "fontSize": 20 } } }
{ "fontSize": 14, "responsive": { "regular": { "fontSize": 20 } } }
```

**After (style file `responsive_body_text.json`):**
```json
{ "fontSize": 14, "responsive": { "regular": { "fontSize": 20 } } }
```

Then apply: `"style": "responsive_body_text"`

### Unnecessary responsive Blocks

Remove responsive blocks that:
- Only override to the same value as default
- Override attributes that have no visual impact at that size
- Are on components that are `visibility: "gone"` with no responsive override to make them visible
...
**REQUIRED: Every Collection MUST have `cellIdProperty` set.** If missing, add it during refactoring. This ensures unique cell identity for efficient diffing and animation. Example: `"cellIdProperty": "id"`. The ViewModel must provide a unique value for this field in each cell's data.
