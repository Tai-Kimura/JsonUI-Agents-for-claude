...
1. Read all files in `<app_config_path>` to understand project requirements:
   - **Library dependencies and versions** → Use for SPM/CocoaPods configuration (e.g., SwiftJsonUI version, third-party libraries)
   - **Import settings** → Determine which modules to import in AppDelegate and ViewControllers
   - **Platform requirements** → Minimum iOS version, deployment targets
   - App name, bundle identifier, etc. → Use for project naming and configuration
2. Apply these settings throughout the setup process (especially dependency and import configuration)
