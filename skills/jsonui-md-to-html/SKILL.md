...
## CRITICAL: Exact 1:1 Conversion Only

**The HTML output must be an EXACT conversion of the markdown file - nothing more, nothing less.**
...
- **NEVER add text, sections, or explanations not in the markdown**
- **NEVER add comments or notes**
- **NEVER add "generated by" or timestamp footers**
- **NEVER add table of contents unless it exists in the markdown**
- **NEVER add any content that does not exist in the source markdown file**

**The ONLY things allowed in the HTML that are not in the markdown:**
1. The HTML template structure (doctype, head, body tags)
2. The CSS styles defined in the template
3. The Mermaid script tag

**Everything inside `<div class="content">` must come ONLY from the markdown file.**

**Before finishing, verify:**
1. All headings from markdown appear in HTML (no extra, no missing)
2. All tables have the exact same number of rows
3. All code blocks are fully converted (no extra, no missing)
4. All paragraphs and lists are present (no extra, no missing)
5. **NO content exists in HTML that was not in the markdown**
...
- **Do NOT add ANY content not in the markdown** - Pure conversion only
- **Do NOT add "Overview", "Summary", or any generated sections**
- **Do NOT add timestamps, version info, or metadata**
