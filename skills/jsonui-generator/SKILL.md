...
Before generating views, read the specification JSON and extract:
- `structure.components` - List of components to generate (may include children, style, binding)
- `structure.layout` - Layout hierarchy (identify root views, collections, includes, overlays)

## Initial Layout Generation with jui

For new screens, you can use `jui generate project` to generate initial Layout JSON and ViewModel scaffolding from the spec:

```bash
jui generate project --file {screen_name}.spec.json
```

This outputs Layout JSON to the shared `layouts_directory` (not per-platform).

For parent_spec (large screens with sub-specs), the command automatically merges sub-specs.

**After `jui generate project`, you still need platform-specific view generation below.**
...
```bash
# SwiftJsonUI
./sjui_tools/bin/sjui g view <ViewName>

# KotlinJsonUI
./kjui_tools/bin/kjui g view <ViewName>

# ReactJsonUI
./rjui_tools/bin/rjui g view <ViewName> [--no-viewmodel]
```

**⛔ DO NOT USE `--root` OPTION:**
- The `--root` option exists but should NOT be used
- Root view configuration is handled separately by project setup
- Always use `g view <ViewName>` without `--root`
...
```bash
# Preferred: distributes layouts from shared directory + builds all platforms
jui build

# Platform-specific (only if debugging a single platform):
./sjui_tools/bin/sjui build   # iOS
./kjui_tools/bin/kjui build   # Android
./rjui_tools/bin/rjui build   # Web
```
...
### Files You CAN Edit:
- JSON layout files in shared `layouts_directory` (e.g., `docs/screens/layouts/*.json`)
